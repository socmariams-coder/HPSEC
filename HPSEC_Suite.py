...
# Configuració volums d'injecció
...
def get_injection_volume(seq_path, is_bp):
    """
    Retorna el volum d'injecció en µL basat en el mode i número de SEQ.

    - BP: sempre 100 µL
    - COLUMN: 400 µL per defecte, 100 µL per SEQs 256-274
    """
    if is_bp:
        return INJECTION_VOLUME_BP

    # Extreure número de SEQ
    seq_name = os.path.basename(seq_path) if seq_path else ""
    match = re.search(r'(\d+)', seq_name)
    if not match:
        return INJECTION_VOLUME_COL_DEFAULT

    seq_int = int(match.group(1))
...
# =============================================================================
# HISTÒRIC DE CALIBRACIÓ KHP
# =============================================================================

# Nom del fitxer d'històric (JSON per compatibilitat amb PowerQuery)
...
# Sistema CALDATA - Carpeta local per cada SEQ amb historial complet
...
def load_local_calibrations(seq_path):
    ...
    Retorna llista de totes les calibracions fetes a aquesta SEQ.
    """
    caldata_path = get_caldata_path(seq_path)
    if not caldata_path:
        return []
...
def get_active_calibration(seq_path, mode=None):
    ...
    Args:
        ...
        mode: Si s'especifica, filtra per mode (COLUMN/BP)
...
def load_khp_history(seq_path):
    ...
    Retorna una llista de diccionaris amb les calibracions.
    """
...
def generate_chromatograms_pdf(seq_path, xlsx_files, is_bp=None, seq_name=None, log_func=None):
    """
    Genera PDF amb tots els cromatogrames de la seqüència (DOC + A254).
    ...
    Args:
        ...
        is_bp: Si és mode BP (opcional, es detecta automàticament si None)
        seq_name: Nom de la SEQ (opcional, s'extreu de seq_path si None)
    ...
    try:
        ...
        pdf_path = os.path.join(check_path, f"REPORT_Cromatogrames_{seq_name}.pdf")
        ...
        for f in sorted(xlsx_files):
            try:
                ...
                mostra = str(id_dict.get("Mostra", os.path.basename(f)))
                rep = str(id_dict.get("Rèplica", "-"))
                method = str(id_dict.get("Method", "COLUMN")).upper()

                # Detectar BP (només cal un fitxer)
                if method == "BP":
                    detected_bp = True
                ...
                y_doc = df_doc["DOC (mAU)"].values if "DOC (mAU)" in df_doc.columns else np.array([])
        ...
        # Definir escala X segons mode
        use_bp = is_bp if is_bp is not None else detected_bp
        if use_bp:
            x_min, x_max = -2, 4
        else:
            x_min, x_max = 0, 70
        ...
        if not chromatograms:
            if log_func:
                log_func(f"  WARN: No s'han trobat cromatogrames per generar PDF")
            return None

        # Generar PDF amb 6 gràfics per pàgina (2 columnes x 3 files)
        graphs_per_page = 6
        n_pages = (len(chromatograms) + graphs_per_page - 1) // graphs_per_page
        ...
        with PdfPages(pdf_path) as pdf:
            for page_idx in range(n_pages):
                ...
                mode_text = " (BP)" if use_bp else ""
                fig.suptitle(f"Cromatogrames SEQ {seq_name}{mode_text} - Pàgina {page_idx + 1}/{n_pages}",
                            fontsize=12, fontweight='bold', y=0.98)

                start_idx = page_idx * graphs_per_page
                end_idx = min(start_idx + graphs_per_page, len(chromatograms))

                for i, chrom in enumerate(chromatograms[start_idx:end_idx]):
                    row = i // 2
                    col = i % 2
                    ax = axes[row, col]
                    ...
                    y_doc = chrom['y_doc']
                    t_dad = chrom['t_dad']
                    y_dad = chrom['y_dad_254']

                    # Plot DOC (blau)
                    ax.plot(t_doc, y_doc, 'b-', linewidth=0.8, label='DOC')
                    ax.set_xlabel('Temps (min)', fontsize=7)
                    ax.set_ylabel('DOC (mAU)', fontsize=7, color='blue')
                    ax.tick_params(axis='y', labelcolor='blue', labelsize=6)
                    ax.tick_params(axis='x', labelsize=6)
                    ax.set_xlim(x_min, x_max)

                    # Títol: mostra + rep
                    title_color = '#28a745' if chrom['is_khp'] else 'black'
                    title_suffix = " [KHP]" if chrom['is_khp'] else ""
                    ax.set_title(f"{chrom['mostra']} R{chrom['rep']}{title_suffix}",
                                fontsize=8, fontweight='bold', color=title_color)

                    # Plot A254 (vermell) si disponible
                    if len(t_dad) > 10 and len(y_dad) > 10:
                        ax2 = ax.twinx()
                        ax2.plot(t_dad, y_dad, 'r-', linewidth=0.6, alpha=0.7, label='A254')
                        ax2.set_ylabel('A254 (mAU)', fontsize=7, color='red')
                        ax2.tick_params(axis='y', labelcolor='red', labelsize=6)

                    ax.grid(True, alpha=0.3, linewidth=0.5)

                # Amagar subplots buits
                for i in range(end_idx - start_idx, graphs_per_page):
                    row = i // 2
                    col = i % 2
        ...
        if log_func:
            log_func(f"  PDF cromatogrames generat: CHECK/REPORT_Cromatogrames_{seq_name}.pdf ({len(chromatograms)} mostres)")

        return pdf_path

    except Exception as e:
        if log_func:
            log_func(f"  WARN: Error generant PDF cromatogrames: {e}")
        return None
...
def _extract_replicas_info(khp_data):
    ...
    Returns:
        Llista de dicts amb info de cada replicat (sense arrays grans)
    """
    replicas = khp_data.get('replicas', khp_data.get('all_khp_data', []))
    if not replicas:
        # Si no hi ha replicas, crear una entrada amb les dades principals
...
def register_calibration(seq_path, khp_data, khp_source, mode="COLUMN"):
    ...
    Args:
        ...
        khp_data: Dades del KHP (àrea, concentració, shift, etc.)
    ...
    # Usar data de la SEQ si disponible, sinó data de processament
    seq_date = khp_data.get('seq_date', '')
    if not seq_date:
        seq_date = datetime.now().isoformat()

    # Calcular volum d'injecció
    is_bp = khp_data.get('is_bp', False)
    volume = get_injection_volume(seq_path, is_bp)

    # Extreure info del pic (límits integració, baseline)
    peak_info = khp_data.get('peak_info', {})

    # Crear entrada amb ID únic
    entry = {
        ...
        # === LÍMITS INTEGRACIÓ (per reproduir exactament) ===
        "t_start": peak_info.get('t_start', khp_data.get('peak_info', {}).get('t_start', 0)),
        "t_end": peak_info.get('t_end', khp_data.get('peak_info', {}).get('t_end', 0)),
        ...
        # === A254 CHECK CALIBRACIÓ ===
        "a254_area": khp_data.get('a254_area', 0),
        "a254_doc_ratio": khp_data.get('a254_doc_ratio', 0),
    }

    # === 1. GUARDAR A LOCAL (CALDATA) - Historial complet ===
    local_cals = load_local_calibrations(seq_path)

    # Desactivar calibracions anteriors del mateix mode
    for cal in local_cals:
        if cal.get("mode") == mode:
            cal["is_active"] = False

    # Afegir nova calibració
    local_cals.insert(0, entry)  # Més recent primer
    save_local_calibrations(seq_path, local_cals)

    # === 2. GUARDAR A GLOBAL (KHP_History.json) - Per comparacions ===
    global_cals = load_khp_history(seq_path)

    # Actualitzar o afegir entrada global (només la activa)
    updated = False
...
def mark_calibration_as_outlier(seq_path, seq_name, mode="COLUMN", is_outlier=True, cal_id=None):
    ...
    Actualitza tant LOCAL (CALDATA) com GLOBAL (KHP_History).
    Si es marca com outlier, activa la següent calibració vàlida.

    Args:
        seq_path: Path per trobar l'històric
        seq_name: Nom de la SEQ a marcar
        mode: COLUMN o BP
        is_outlier: True per marcar com outlier, False per desmarcar
        cal_id: ID específic de la calibració (opcional, si no s'especifica usa l'activa)

    Returns:
        True si s'ha trobat i actualitzat, False altrament
    """
    # === 1. ACTUALITZAR LOCAL (CALDATA) ===
    local_cals = load_local_calibrations(seq_path)
    local_updated = False
    target_cal = None
    ...
    # Si es marca com outlier, activar la següent calibració vàlida
    new_active = None
    if is_outlier and local_updated:
        for cal in local_cals:
    ...
    # === 2. ACTUALITZAR GLOBAL (KHP_History) ===
    global_cals = load_khp_history(seq_path)
    global_updated = False

    for i, cal in enumerate(global_cals):
        if cal.get("seq_name") == seq_name and cal.get("mode") == mode:
            if is_outlier and new_active:
                # Substituir per la nova activa
                global_cals[i] = new_active.copy()
            else:
                cal["is_outlier"] = is_outlier
            global_updated = True
            break

    # Si no hi ha nova activa i es marca outlier, eliminar del global
    if is_outlier and not new_active and global_updated:
        global_cals = [c for c in global_cals
                       if not (c.get("seq_name") == seq_name and c.get("mode") == mode)]

    if global_updated or (is_outlier and not new_active):
        save_khp_history(seq_path, global_cals)
...
def extract_khp_conc(filename):
    """Extreu concentració KHP del nom de fitxer."""
    match = re.search(r"KHP\s*(\d+)", filename, re.IGNORECASE)
    if match:
...
def calculate_peak_symmetry(t, y, peak_idx, left_idx, right_idx):
    """Calcula factor de simetria del pic (USP tailing factor)."""
    try:
        # Simetria al 10% de l'altura
        peak_height = y[peak_idx]
        height_10pct = peak_height * 0.1

        # Trobar punts al 10% a esquerra i dreta
        left_seg = y[left_idx:peak_idx+1]
        right_seg = y[peak_idx:right_idx+1]

        # Interpolació per trobar temps exactes
        t_left_10 = None
        t_right_10 = None

        for i in range(len(left_seg)-1):
            if left_seg[i] <= height_10pct <= left_seg[i+1]:
                t_left_10 = t[left_idx + i]
                break

        for i in range(len(right_seg)-1):
            if right_seg[i] >= height_10pct >= right_seg[i+1]:
                t_right_10 = t[peak_idx + i]
                break

        if t_left_10 is None or t_right_10 is None:
            return 1.0

        t_peak = t[peak_idx]
        a = t_peak - t_left_10  # Front
        b = t_right_10 - t_peak  # Tail

        if a <= 0:
            return 1.0

        # Tailing factor = (a + b) / (2 * a)
        return (a + b) / (2 * a)
...
def calculate_peak_snr(y, peak_idx, baseline_mean, baseline_std):
    """Calcula Signal-to-Noise Ratio del pic."""
    if baseline_std <= 0:
        return 0.0
    return (y[peak_idx] - baseline_mean) / baseline_std
...
def find_peak_boundaries(t, y, peak_idx, baseline_level=None, threshold_pct=5.0, is_bp=False):
    ...
    Busca on el senyal baixa fins al threshold_pct de l'alçada del pic
    sobre la baseline, o fins a un mínim local.

    Args:
        t: array de temps
        y: array de senyal
        peak_idx: índex del màxim del pic
        baseline_level: nivell de baseline (si None, es calcula)
        threshold_pct: percentatge de l'alçada per definir límits (default 5%)
        is_bp: si és mode BP (pic primerenc, baseline al final)
    ...
    # Calcular baseline si no es proporciona
    if baseline_level is None:
        if is_bp:
            # BP: El pic surt molt aviat, usar els ÚLTIMS punts per baseline
            # (després del pic, el senyal torna a baseline)
            n_edge = max(20, n // 5)  # Últim 20%
            baseline_level = np.median(y[-n_edge:])
        else:
            # COLUMN: Usar el mínim dels primers i últims 10%
            n_edge = max(10, n // 10)
    ...
    # Threshold: baseline + threshold_pct% de l'amplitud
    ...
    for i in range(peak_idx - 1, -1, -1):
        if y[i] <= threshold:
            left_idx = i
            break
        # També parar si trobem un mínim local (vall)
        if i > 0 and y[i] < y[i-1] and y[i] < y[i+1]:
            left_idx = i
            break
        left_idx = i

    # Per BP: si el límit esquerre és index 0 però t[0] > 0, el pic pot començar abans!
    # Avisar que potser es perd senyal
    if is_bp and left_idx == 0 and t[0] > 0.1:
        # El pic comença al primer punt de dades - pot ser que es perdi la pujada
        # Això és informatiu, el codi ja captura des del primer punt disponible
        pass
...
def detect_all_peaks(t, y, min_prominence_pct=5.0, is_bp=None):
    """Detecta tots els pics amb prominència mínima i límits d'integració correctes."""
    t = np.asarray(t, dtype=float)
    y = np.asarray(y, dtype=float)
    ...
    # Calcular baseline per a tot el senyal
    n = len(y)
    if is_bp:
    ...
    min_prom = y_max * (min_prominence_pct / 100.0)
    peaks, props = find_peaks(y, prominence=min_prom, width=3)

    result = []
    for i, pk in enumerate(peaks):
        # Usar find_peak_boundaries per límits correctes
        left_idx, right_idx = find_peak_boundaries(t, y, int(pk), baseline_level, threshold_pct=5.0, is_bp=is_bp)

        # Assegurar que tenim rang vàlid
        left_idx = max(0, left_idx)
...
def buscar_khp_consolidados(input_folder):
    """
...
def analizar_khp_consolidado(khp_file, config=None):
    ...
    Retorna dict amb conc, area, shift_min, qualitat, dades per gràfics, etc.
    """
    ...
    try:
        ...
        # Llegir metadades del full ID (DOC_MODE, Method, Date, etc.)
        doc_mode = "N/A"
        seq_date = ""
        method_from_file = None  # BP o COLUMN del fitxer
        if "ID" in xls.sheet_names:
            ...
            # Llegir mètode (BP o COLUMN) del fitxer si existeix
        ...
        # Temps de retenció del màxim del pic (FONAMENTAL en cromatografia)
        t_retention = peak_info.get('t_max', 0)

        # Detectar si és BP - prioritat: 1) Method del fitxer, 2) nom, 3) cromatograma
        ...
        if method_from_file == "BP":
            ...
            # Fallback: detectar per nom o cromatograma
        ...
        # Baseline stats - usar finestra temporal per evitar incloure el pic
        if is_bp_chromato:
            # BP: baseline dels primers 2 min (abans del pic KHP ~4-6 min)
            bl_stats = baseline_stats_time(t_doc, doc_net, t_start=0, t_end=2.0)
        else:
            # COLUMN: baseline dels primers 10 min (abans del pic KHP ~20-30 min)
            bl_stats = baseline_stats_time(t_doc, doc_net, t_start=0, t_end=10.0)

        # Qualitat del pic principal - usar índexs de peak_info
        ...
        # Si no tenim límits vàlids, buscar en all_peaks
        for pk in all_peaks:
        ...
        # ============================================================
        # IMPORTANT: Expandir límits si no arriben a la baseline
        # Això corregeix pics amb "orelles" o "batman" on els límits
        # inicials són massa estrets
        # ============================================================
        ...
        expansion = expand_integration_limits_to_baseline(
            ...
            min_width_minutes=1.0,  # Mínim 1 minut d'amplada
            max_width_minutes=6.0 if is_bp_chromato else 10.0,  # Màxim segons mode
        ...
        expanded_left = expansion['expanded_left']
        expanded_right = expansion['expanded_right']

        # Si s'han expandit els límits, recalcular l'àrea
        if limits_expanded:
            # Recalcular àrea amb els nous límits
            from scipy.integrate import trapezoid
            new_area = float(trapezoid(doc_net[left_idx:right_idx+1], t_doc[left_idx:right_idx+1]))
            old_area = peak_info.get('area', 0)

            # Actualitzar peak_info
            ...
            peak_info['expansion_info'] = {
                ...
                'expanded_left_points': expanded_left,
                'expanded_right_points': expanded_right,
                'old_area': old_area,
                'new_area': new_area,
                'area_increase_pct': ((new_area - old_area) / old_area * 100) if old_area > 0 else 0
            }

        # Simetria (tailing factor) - amb límits corregits
        ...
        if has_dad:
            ...
            if "254" in df_dad.columns:
                ...
                # Integrar A254 usant finestra temporal basada en límits DOC
                if dad_peak_info and dad_peak_info.get('valid') and len(t_dad) > 10:
                    from scipy.integrate import trapezoid

                    # Usar temps d'integració del DOC (amb correcció de shift)
                    t_start_doc = peak_info.get('t_start', t_doc[left_idx])
                    t_end_doc = peak_info.get('t_end', t_doc[right_idx])

                    # Trobar índexs equivalents en el senyal DAD (ajustat per shift)
                    ...
                    if dad_right_idx > dad_left_idx:
                        ...
                        # Ratio DOC/A254 (útil per validació creuada)
                        if a254_area > 0:
                            a254_doc_ratio = peak_info['area'] / a254_area

                        # Guardar límits d'integració A254
                        dad_peak_info['a254_area'] = a254_area
                        dad_peak_info['a254_left_idx'] = dad_left_idx
                        dad_peak_info['a254_right_idx'] = dad_right_idx
                        dad_peak_info['a254_t_start'] = float(t_dad[dad_left_idx])
                        dad_peak_info['a254_t_end'] = float(t_dad[dad_right_idx])
        ...
        # Informar si els límits han estat expandits (correccio automàtica)
        baseline_valid = True  # Ara sempre és vàlid perquè s'ha corregit
        if limits_expanded:
            ...
            # No penalitzar score ja que s'ha corregit automàticament
        ...
        return {
            ...
            'is_bp': is_bp_chromato,  # Indica si és mode BP (per decisions d'alineació)
            # Metadades del fitxer consolidat
            'doc_mode': doc_mode,  # UIB o DIRECT
            'seq_date': seq_date,  # Data de la seqüència
            # Temps de retenció (FONAMENTAL)
            't_retention': t_retention,  # Temps al màxim del pic en minuts
            # Expansió de límits d'integració
            'baseline_valid': baseline_valid,
            'limits_expanded': limits_expanded,
            'expansion_info': expansion,
            # A254 check de calibració
            'a254_area': a254_area,
...
def analizar_khp_lote(khp_files, config=None):
    ...
    Retorna dades detallades de tots els KHP + selecció final.
    """
    all_data = []
    for f in khp_files:
    ...
    data = all_data  # Mantenim referència
    ...
    # Seleccionar millor: preferir menor quality_score, si igual, major àrea
    if rsd < 10.0:
        ...
        result = {
            ...
            'stats': stats,
            'is_average': True,
            'selected_idx': -1,  # -1 indica promig
            # Per gràfics, usar primera rèplica
            ...
            # A254 check (promig de rèpliques)
...
# =============================================================================
# FUNCIONS CONSOLIDACIÓ (de CON_v1.py)
# =============================================================================
def normalize_key(s):
...
class HPSECSuite:
    ...
    def _consolidation_thread(self):
        """Thread de consolidació - Implementació completa UIB/DIRECT."""
        try:
            input_folder = self.seq_path
            ...
            if mode == "UIB":
                ...
                for i, f_doc in enumerate(uib_files):
                    ...
                    self.root.after(0, lambda p=progress, s=mostra: self._update_con_progress(p, s))
                    ...
                    base = get_baseline_correction(t_doc, y_doc, method, self.config)
                    ...
                    if dad_path:
                        ...
                        if st_read.startswith("OK") and not df_dad_raw.empty:
                            df_dad = process_dad(df_dad_raw, self.config)
                            st_dad = "OK" if not df_dad.empty else "NO_DATA"

                    # Detectar pic
                    peak_info = detect_main_peak(t_doc, doc_net, self.config["peak_min_prominence_pct"])

                    # Calcular àrees (DOC + DAD wavelengths)
                    sample_analysis = analyze_sample_areas(t_doc, doc_net, df_dad, peak_info, self.config)

                    # Guardar Excel
                    out_name = f"{mostra}_{seq_out}_R{rep}.xlsx"
                    out_path = os.path.join(path_out, out_name)

                    self._write_consolidated_excel(
                    ...
                    sample_entry = create_sample_entry(
                        self.seq_path, mostra, rep, date_master, method, "UIB",
                        sample_analysis
                    )
                ...
                if not mestre:
                    self.root.after(0, lambda: self._consolidation_error("No s'ha trobat Excel mestre"))
                    return
                ...
                for col in needed:
                    if col not in df_seq.columns:
                        self.root.after(0, lambda c=col: self._consolidation_error(f"Falta columna: {c}"))
                        return
                ...
                for i, (_, row) in enumerate(df_seq2.iterrows()):
                    ...
                    self.root.after(0, lambda p=progress, s=mostra_clean: self._update_con_progress(p, s))
                    ...
                    base_arr = get_baseline_correction(t_doc, y_doc, method, self.config)
                    ...
                    # Afegir a llista de mostres
                    sample_entry = create_sample_entry(
                        self.seq_path, mostra_clean, rep, date_master, method, "DIRECT",
                        sample_analysis
                    )
                    sample_entries.append(sample_entry)
            ...
            if sample_entries:
                try:
                    add_samples_batch(self.seq_path, sample_entries, replace_seq=True)
                except Exception as e:
                    print(f"WARN: Error guardant mostres a històric: {e}")

            self.consolidation_count = processed_count
    ...
    def _consolidation_done(self):
        ...
        # Generar PDF de control
        self._generate_consolidation_pdf(info, xlsx_files)

        # Generar PDF de cromatogrames
        self._generate_chromatograms_pdf(info, xlsx_files)
    ...
    def _generate_consolidation_pdf(self, info, xlsx_files):
        """Genera PDF de control de consolidació amb matplotlib PdfPages."""
        try:
            check_path = ensure_check_folder(self.seq_path)
            seq_name = info.get('seq', obtenir_seq(self.seq_path))
            pdf_path = os.path.join(check_path, f"REPORT_Consolidacio_{seq_name}.pdf")

            print(f"Generant PDF consolidació a: {pdf_path}")
            ...
            with PdfPages(pdf_path) as pdf:
                ...
                dad_ok = sum(1 for f in xlsx_files if self._check_dad_status(f))
                ...
                summary_data = [
                    ["CONFIGURACIÓ", "VALOR"],
                    ["Carpeta origen", os.path.basename(self.seq_path)],
                ...
                if info.get('bp', False):
                    baseline_info = f"Mode BP: Finestra 0-{self.config['bp_baseline_win']} min"
                else:
                    baseline_info = f"Mode COLUMN: Finestra 0-{self.config['col_baseline_start']} min"
                ...
                # Crear taula de parells (sense columna DAD OK)
                headers = ["Mostra", "Rep", "Font DOC", "Fitxer DAD", "Pts DOC", "Pts DAD"]
                rows = []
                row_status = []  # Per marcar files amb problemes
                ...
                for f in sorted(xlsx_files)[:35]:
                    try:
                        ...
                        # Font DOC: per DIRECT mostrar fitxer i files
                        if info.get('mode') == 'UIB':
                            doc_file = str(id_dict.get("Fitxer_DOC_Original", "-"))
                            doc_file = doc_file[-20:] if len(doc_file) > 20 else doc_file
                        else:
                            # Mode DIRECT: mostrar fitxer mestre i files
                            master_file = str(id_dict.get("Fitxer_Master", ""))
                            if master_file:
                                master_name = os.path.basename(master_file)[-15:] if len(master_file) > 15 else os.path.basename(master_file)
                            else:
                                master_name = "Excel"
                ...
                if rows:
                    ...
                    for i, (row, status) in enumerate(zip(rows, row_status), start=1):
                        if '*' in row[0]:  # KHP
                            ...
                            # Marcar en vermell les files amb dades faltants
                            for j in range(len(headers)):
                                tbl2[(i, j)].set_facecolor('#f8d7da')
                            # Marcar específicament cel·les amb MISSING
            ...
            if os.path.exists(pdf_path):
                print(f"PDF consolidació generat correctament: {pdf_path}")
                self.root.after(0, lambda p=pdf_path: messagebox.showinfo("PDF Consolidació",
                    f"PDF de consolidació generat:\n{p}"))
            else:
                raise Exception(f"El fitxer no s'ha creat: {pdf_path}")

        except Exception as e:
            import traceback
            error_msg = traceback.format_exc()
            print(f"Error generant PDF consolidació: {error_msg}")
            self.root.after(0, lambda err=str(e): messagebox.showerror("Error PDF",
                f"Error generant PDF de consolidació:\n{err}"))
    ...
    def _generate_chromatograms_pdf(self, info, xlsx_files):
        """Genera PDF amb tots els cromatogrames - wrapper a funció standalone."""
        is_bp = info.get('bp', False)
        seq_name = info.get('seq', obtenir_seq(self.seq_path))
        generate_chromatograms_pdf(self.seq_path, xlsx_files, is_bp=is_bp, seq_name=seq_name)
