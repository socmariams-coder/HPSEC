...
def _normalize_key(s: str) -> str:
    """
    Normalitza string per matching (consistent amb hpsec_import.normalize_key).

    - Elimina guions (-) i espais
    - MANTÉ underscores (_) perquè són significatius
    - Converteix a majúscules
    """
    import re
    result = re.sub(r"[\-\s]+", "", str(s or ""))
    return result.upper()


def _create_sample_rep(df_hplc: pd.DataFrame, sample_col: str) -> List[str]:
    """
    Crea Sample_Rep amb format simple: NOM_R{inj_num}

    Només neteja el nom (elimina espais i guions) i afegeix _R1, _R2, etc.
    NO afegeix numeració de blocs (_B1, _B2) ni convenció Agilent (NOM1, NOM2).

    Exemples:
        LQ0149      -> LQ0149_R1, LQ0149_R2
        LQ0149_50   -> LQ0149_50_R1, LQ0149_50_R2
        NaOH 0.1mM  -> NAOH0.1MM_R1, NAOH0.1MM_R2
    """
    import re
    ...
    def clean_name(s):
        """Neteja nom: elimina espais i guions, manté underscores, majúscules."""
        cleaned = re.sub(r"[\s\-]+", "", str(s or "").strip())
        return cleaned.upper()

    if not inj_num_col:
        return [f"{clean_name(row[sample_col])}_R1" for _, row in df_hplc.iterrows()]

    result = []
    for _, row in df_hplc.iterrows():
        sample_name = row[sample_col]
        inj_num = int(row[inj_num_col]) if pd.notna(row[inj_num_col]) else 1
        cleaned = clean_name(sample_name)
        result.append(f"{cleaned}_R{inj_num}")
